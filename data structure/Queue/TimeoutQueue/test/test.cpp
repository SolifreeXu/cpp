...
int main()
{
	...
	using Functor = std::function<const ElementType* ()>;
	using QueueType = TimeoutQueue<const ElementType*, Functor, std::time_t>;
	...
	for (const auto& element : array)
	{
		...
		queue.push(address, [address] { return address; }, time);
	}
	...
	if (queue.pop(std::time(nullptr), vector))
		for (const auto& [address, functor] : vector)
			std::cout << address << ' ' << *functor() << std::endl;
